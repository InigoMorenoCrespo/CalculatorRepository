...
private extension ViewModel {
    ...
    private func didSelectNumber(with number: Int) {
            if self.currentNumber == .firstNumber {
                if let firstNumber = self.firstNumber, firstNumber == "0" || firstNumber == self.getOperarionResult(self.operation ?? .add, firstNumber.toDouble, self.prevNumber?.toDouble).description {
                ...
                if let secondNumber = self.secondNumber, secondNumber == "0" || secondNumber == self.getOperarionResult(self.operation ?? .add, secondNumber.toDouble, self.prevNumber?.toDouble).description {
